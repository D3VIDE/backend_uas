<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["kota"]) && isset($_POST["keterangan"]) && isset($_POST["nomor_resi_log"]) && isset($_POST["tanggal_log"])) {
      $kota = $_POST["kota"];
      $keterangan = $_POST["keterangan"];
      $tanggalLog = $_POST["tanggal_log"]; 
      $noResiLog = $_POST["nomor_resi_log"];

      // Check if nomor_resi exists in transaksi_resi_pengiriman (optional validation)
      ...
      if ($resultCheck->num_rows > 0) {
          // Insert a new log entry instead of updating existing ones
          $insertLogQuery = "INSERT INTO detail_log_pengiriman (nomor_resi, kota, keterangan, tanggal) VALUES (?, ?, ?, ?)";
          $stmtInsertLog = $conn->prepare($insertLogQuery);
          $stmtInsertLog->bind_param("ssss", $noResiLog, $kota, $keterangan, $tanggalLog);
          $stmtInsertLog->execute();
          header("Location: admin_page.php");
          exit();
      } else {
          $alertMessage = "Nomor resi tidak ditemukan dalam database.";
      }
  }
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="#" method="POST">
            ...
            <!-- Tambahan input untuk tanggal -->
            <div class="mb-3">
              <label for="tanggal_log" class="form-label">Tanggal</label>
              <input type="date" class="form-control" id="tanggal_log" name="tanggal_log" required>
            </div>
